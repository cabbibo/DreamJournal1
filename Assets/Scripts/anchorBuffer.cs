...
//
  ...
  void createAnchorBuffer(){
    ...
    for (int i = 0; i < triangles.Length; i++) {
      ...
      int id           = triangles[i];
      int id1 = triangles[(i+1)%3];
      int id2 = triangles[(i+2)%3];
...
//      if( i  % 100 == 0 ){ print( tangent ); }
      ...
      inValues[index++] = (float)id;
      inValues[index++] = (float)id1;
      inValues[index++] = (float)id2;



      inValues[index++] = (float)i;
